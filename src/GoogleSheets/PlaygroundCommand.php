...
//        $this->dotEnv->required("ADEO_JIRA_EMAIL");
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $client->setAuthConfig(BASE_PATH . DIRECTORY_SEPARATOR. $_SERVER['GOOGLE_SHEETS_AUTH_JSON']);

        $service = new \Google_Service_Sheets($client);

        $spreadsheetId = $_SERVER['GOOGLE_SHEETS_SPREADSHEET_ID'];

        $fh = fopen('report.csv', 'r');
        $data = [];
        ...
        $range = $_SERVER['GOOGLE_SHEETS_UPDATE_RANGE']; // the service will detect the last row of this sheet
